...
public:
  ...
  {
    return make_signal( 0, !value ); // constant is 1 for all edges (node 0), complemented (value = false) or not complemented (value = true)
  }
  ...
  signal_t unique( var_t var, signal_t T, signal_t E )
  {
    ...
    {
      T = make_signal( get_index(T), false );
      E = make_signal( get_index(E), !is_complemented(E) );
      output_neg = true;
    }
  ...
  /*********************** Ref & Deref **********************/
  /**********************************************************/
  signal_t ref( signal_t f ) /* increases ref count of node f and returns f itself */ 
  {
    refs[ get_index(f) ] += 1;
    return f;
  }

  void deref( signal_t f ) /* decreases ref count of node f, recursively deref f's children if f becomes dead */ 
  {
      refs[ get_index(f) ] -= 1;
      Node F = get_node(f);
  ...
  signal_t XOR( signal_t f, signal_t g )
  {
    ...
    for( auto p = computed_table_XOR.begin(); p != computed_table_XOR.end(); p++ )
    {
      if( p->first == std::make_tuple(f,g) || p->first == std::make_tuple(g,f) )
      {
        return p->second;
      }
    }
  ...
  signal_t AND( signal_t f, signal_t g )
  {
    ...
    for( auto p = computed_table_AND.begin(); p != computed_table_AND.end(); p++ )
    {
      if( p->first == std::make_tuple(f,g) || p->first == std::make_tuple(g,f) )
      {
        return p->second;
  ...
  signal_t OR( signal_t f, signal_t g )
  {
    ...
    for( auto p = computed_table_OR.begin(); p != computed_table_OR.end(); p++ )
    {
      if( p->first == std::make_tuple(f,g) || p->first == std::make_tuple(g,f) )
      {
        return p->second;
  ...
  signal_t ITE( signal_t f, signal_t g, signal_t h )
  {
    ...
    for( auto p = computed_table_ITE.begin(); p != computed_table_ITE.end(); p++ )
    {
